...
exports.getImagesSizes = function (files, options, callback) {
  ...
  const promiseChunks = _.chunk(filePaths, 10).map((chunkedFiles, chunk) => {
    var promiseResolve, promiseReject;
    var promise = new Promise((resolve, reject) => {
      promiseResolve = resolve;
      promiseReject = reject;
    })
    exec('identify ' + chunkedFiles.join(' '), function (err, stdout) {
      if (err) return promiseReject(new Error('Execution of identify command failed. Ensure that ImageMagick Legacy Tools are installed and added to your PATH.'));

      var sizes = stdout.split('\n');
      var chunkStartPos = chunk * 10;
      sizes = sizes.splice(0, sizes.length - 1);
      sizes.forEach(function (item, fileChunkIndex) {
        var i = chunkStartPos + fileChunkIndex;
  ...
  Promise.all(promiseChunks).then(() => {
    callback(null, files);
  }).catch((er) => callback(err))
};
...
function makeCommands(fileList, options, index) {
  return [
    ...
    index !== 0 && `-page +0+0 ${options.path}/${options.name}.png`,
    ...fileList.map(
      file => `-page +${(file.x + options.padding)}+${(file.y + options.padding)} "${file.path}"`,
    ),
    '-layers merge',
...
function makeCommandFinish(options) {
  return [
...
exports.generateImage = function (files, options, callback) {
  ...
  var runAllCommands = async () => {
    let i = 0;
    const all = commands.length + 1;
    const logProgress = () => console.log(`${i++} / ${all}`)
    for (command of commands) {
      logProgress()
      await execPromise(command);
    }
    logProgress();
    await execPromise(makeCommandFinish(options))
  }
